-- Add Other Ignition Method to Burn Project

UPDATE `ignition_methods`
SET `name` = 'Other Ignition Method'
WHERE `name` = 'Other';

ALTER TABLE `burn_projects`
ADD COLUMN `other_ignition_method` text;

INSERT INTO `fields` (`table_id`, `column`, `datatype`, `sql`, `primary_key`, `allow_null`)
VALUES (5, 'other_ignition_method', 1, NULL, 0, 1);


-- Annual Reports Views

DROP VIEW yearly_project_fuels;
CREATE VIEW yearly_project_fuels
AS SELECT a.burn_project_id, YEAR(start_datetime) as year,
  SUM(f.total_tons) as tons_consumed, SUM(f.black_acres) as black_acres,
  SUM(f.tons_emitted) as tons_emitted,  fu.emission_type_id
...
WHERE a.status_id = 4
GROUP BY a.burn_project_id, YEAR(start_datetime), emission_type_id;


DROP VIEW yearly_project_fuels_wide;
